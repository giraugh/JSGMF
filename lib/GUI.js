...
gui = function() {
  ...
  ret = {
    ...
    update: function() {
      var i, isHovering, mouse, op, ref, results, shape, updateOperations;
      mouse = {
      ...
      updateOperations = (function(_this) {
        return function(op, id) {
          ...
          op.current++;
          if (op.current === op.time) {
            return _this._operations.splice(id, 1);
          }
        };
      })(this);
      results = [];
      for (op = i = 0, ref = this._operations.length; 0 <= ref ? i < ref : i > ref; op = 0 <= ref ? ++i : --i) {
        results.push(updateOperations(this._operations[op], op));
      }
      return results;
    },
    draw: function(ctx) {
